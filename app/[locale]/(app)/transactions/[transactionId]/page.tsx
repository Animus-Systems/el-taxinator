...
export default async function TransactionPage({ params }: { params: Promise<{ locale: string; transactionId: string }> }) {
  ...
  const files = await getFilesByTransactionId(transactionId, transaction.userId)
  const [categories, currencies, settings, fields, projects] = await Promise.all([
