...
import type { Category, Field, File, Project, Transaction } from "@/lib/db-types"
...
/**
 * Split a bank statement into items AND immediately save all as transactions.
 * Skips the review step for maximum speed — useful when AI categorization is trusted.
 */
export async function splitAndSaveAllAction(
  ...
  try {
    ...
    // Process items in batches of 5 for concurrency
    const BATCH_SIZE = 5
    for (let i = 0; i < items.length; i += BATCH_SIZE) {
      ...
      await Promise.all(batch.map(async (item, idx) => {
        ...
        const transaction = await createTransaction(user.id, txData)

        const fileUuid = randomUUID()
        const fileName = `${originalFile.filename}-${item.name ?? (i + idx)}`
    ...
    console.error("Failed to split and save all:", error)
