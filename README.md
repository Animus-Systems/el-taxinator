...
- **Always-on chat assistant** — A floating, draggable, resizable chat panel sits on every page. Ask questions about any transaction, propose a new rule, apply a rule to existing rows, bulk-update transactions, create categories/projects on the fly, or get step-by-step walkthroughs of how to file each Spanish modelo on AEAT / Agencia Tributaria Canaria — portal URLs, login options, which casillas to fill, NRC payment flow, common errors. Rolling 100-message history with automatic summarisation keeps context without bloating the prompt.
- **Living Spanish tax knowledge packs** — Canary Islands autónomo and SL tax theory (IGIC rates, Modelo deadlines, deductibility rules) plus **per-modelo filing procedures** (Modelo 420 / 130 / 202 / 100 / 425 / 721 — step-by-step portal walkthroughs) ship as markdown seeds. Refresh one pack at a time, batch-refresh stale packs, or force-refresh everything with your own configured LLM — no paid web-search provider needed. Refreshed packs land as `needs_review` until you mark them verified. The filing packs are also injected into the chat assistant's prompt so it can cite the exact portal flow.
...
- **Tax calculator dashboard redesign** — Action-first UI: a "next deadline" hero picks the soonest unfiled quarter, a four-step timeline strip shows filed / current / upcoming / overdue at a glance, per-quarter cards consolidate IGIC + IRPF/Corp in one place, and an annual totals strip replaces the old summary card. Each modelo detail page gets a filing hero (amount to pay, deadline, agency, Mark-as-filed), grouped casilla tables with click-to-drill-down into the underlying invoices / expenses, a persisted per-user filing checklist, and a link to the matching filing-procedure knowledge pack.
